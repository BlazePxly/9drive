import { CheckSquare, Download, Edit3, Eye, FolderInput, Info, Link2, Trash2, UserPlus } from 'lucide-react'
import { Button } from '@/components/ui/button'
import type { FileItem } from '@/data/drive-data'

export function FileContextMenu({ x, y, file, onClose, onSelect, onView, onDownload, onRename, onMove, onDetails, onShare, onInvite, onDelete }: { x: number; y: number; file: FileItem | null; onClose: () => void; onSelect?: () => void; onView: () => void; onDownload: () => void; onRename: () => void; onMove: () => void; onDetails: () => void; onShare: () => void; onInvite: () => void; onDelete: () => void }) {
  if (!file) return null
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-3 bottom-3 z-50 rounded-3xl border border-slate-800 bg-slate-900 p-2 shadow-2xl shadow-slate-900/15 sm:inset-x-auto sm:bottom-auto sm:w-52 sm:rounded-2xl" style={{ left: window.innerWidth >= 640 ? safeX : undefined, top: window.innerWidth >= 640 ? safeY : undefined }}>
        <p className="truncate px-3 py-2 text-xs font-bold text-slate-400">{file.name}</p>
        {onSelect && <Button variant="ghost" className="h-12 w-full justify-start sm:h-11" onClick={onSelect}><CheckSquare className="h-4 w-4" />Select</Button>}
