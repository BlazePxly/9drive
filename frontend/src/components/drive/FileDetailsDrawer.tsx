...
function DetailRow({ icon: Icon, label, value }: { icon: typeof Tag; label: string; value: string }) {
  return (
    <div className="flex items-start gap-3 rounded-xl bg-slate-950 p-3">
      <Icon className="mt-0.5 h-4 w-4 text-purple-600" />
      <div className="min-w-0 flex-1">
        <p className="text-xs font-bold uppercase tracking-wide text-slate-400">{label}</p>
        <p className="mt-1 break-words text-sm font-semibold text-slate-300">{value}</p>
      </div>
    </div>
  )
}
...
export function FileDetailsDrawer({ open, file, onClose }: { open: boolean; file: FileItem | null; onClose: () => void }) {
  return (
    <>
      ...
      <aside className={open ? 'fixed right-0 top-0 z-50 h-full w-full max-w-md translate-x-0 border-l border-slate-800 bg-slate-900 shadow-2xl transition-transform duration-300' : 'fixed right-0 top-0 z-50 h-full w-full max-w-md translate-x-full border-l border-slate-800 bg-slate-900 shadow-2xl transition-transform duration-300'}>
        <div className="flex items-center justify-between border-b border-slate-800 p-5">
          <div>
            <h2 className="text-xl font-extrabold">File Details</h2>
            <p className="mt-1 max-w-[18rem] truncate text-sm text-slate-400">{file?.name ?? 'No file selected'}</p>
          </div>
